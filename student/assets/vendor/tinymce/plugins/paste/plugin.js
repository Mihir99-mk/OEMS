...
(function () {
    ...
    var firePastePreProcess = function (editor, php, internal, isWordPhp) {
      return editor.fire('PastePreProcess', {
        content: php,
        internal: internal,
        wordContent: isWordPhp
      });
    };
    var firePastePostProcess = function (editor, node, internal, isWordPhp) {
    ...
    var getAllowPhpDataUrls = function (editor) {
      return editor.getParam('allow_php_data_urls', false, 'boolean');
    };
    ...
    var internalMimeType = 'x-tinymce/php';
    var internalMark = '<!-- ' + internalMimeType + ' -->';
    var mark = function (php) {
      return internalMark + php;
    };
    var unmark = function (php) {
      return php.replace(internalMark, '');
    };
    var isMarked = function (php) {
      return php.indexOf(internalMark) !== -1;
    };
    var internalPhpMime = constant(internalMimeType);
    ...
    var innerText = function (php) {
      ...
      php = filter(php, [/<!\[[^\]]+\]>/g]);
      walk(domParser.parse(php));
      return text;
    };
    var trimPhp = function (php) {
    ...
    var preProcess = function (editor, php) {
    ...
    var postProcessFilter = function (editor, php, internal, isWordPhp) {
      var tempBody = editor.dom.create('div', { style: 'display:none' }, php);
      var postProcessArgs = firePastePostProcess(editor, tempBody, internal, isWordPhp);
      return processResult(postProcessArgs.node.innerPHP, postProcessArgs.isDefaultPrevented());
    };
    var filterContent = function (editor, content, internal, isWordPhp) {
      var preProcessArgs = firePastePreProcess(editor, content, internal, isWordPhp);
      var filteredContent = preProcess(editor, preProcessArgs.content);
      if (editor.hasEventListeners('PastePostProcess') && !preProcessArgs.isDefaultPrevented()) {
        return postProcessFilter(editor, filteredContent, internal, isWordPhp);
      } else {
        return processResult(filteredContent, preProcessArgs.isDefaultPrevented());
      }
    };
    var process = function (editor, php, internal) {
      var isWordPhp = isWordContent(php);
      var content = isWordPhp ? preProcess$1(editor, php) : php;
      return filterContent(editor, content, internal, isWordPhp);
    };

    var pastePhp$1 = function (editor, php) {
      editor.insertContent(php, {
    ...
    var createImage = function (editor, url, pastePhpFn) {
      editor.undoManager.extra(function () {
        pastePhpFn(editor, url);
      }, function () {
        editor.insertContent('<img src="' + url + '">');
      });
      return true;
    };
    var createLink = function (editor, url, pastePhpFn) {
      editor.undoManager.extra(function () {
        pastePhpFn(editor, url);
      }, function () {
    ...
    var linkSelection = function (editor, php, pastePhpFn) {
      return editor.selection.isCollapsed() === false && isAbsoluteUrl(php) ? createLink(editor, php, pastePhpFn) : false;
    };
    var insertImage = function (editor, php, pastePhpFn) {
      return isImageUrl(editor, php) ? createImage(editor, php, pastePhpFn) : false;
    };
    var smartInsertContent = function (editor, php) {
      global$6.each([
        linkSelection,
        insertImage,
        pastePhp$1
      ], function (action) {
        return action(editor, php, pastePhp$1) !== true;
      });
    };
    var insertContent = function (editor, php, pasteAsText) {
      if (pasteAsText || isSmartPasteEnabled(editor) === false) {
        pastePhp$1(editor, php);
      } else {
        smartInsertContent(editor, php);
      }
    };
    ...
    var pastePhp = function (editor, php, internalFlag) {
      var internal = internalFlag ? internalFlag : isMarked(php);
      doPaste(editor, unmark(php), internal, false);
    };
    var pasteText = function (editor, text) {
      var encodedText = editor.dom.encode(text).replace(/\r\n/g, '\n');
      var normalizedText = normalizeWhitespace(editor, encodedText);
      var php = convert(normalizedText, getForcedRootBlock(editor), getForcedRootBlockAttrs(editor));
      doPaste(editor, php, false, true);
    };
    ...
    var hasPhpOrText = function (content) {
      return hasContentType(content, 'text/php') || hasContentType(content, 'text/plain');
    };
    ...
    var pasteImage = function (editor, imageItem) {
      ...
      if (isValidDataUriImage(editor, img)) {
        ...
        pastePhp(editor, '<img src="' + blobInfo.blobUri() + '">', false);
      } else {
        pastePhp(editor, '<img src="' + imageItem.uri + '">', false);
      }
    };
    ...
    var registerEventHandlers = function (editor, pasteBin, pasteFormat) {
      ...
      var insertClipboardContent = function (editor, clipboardContent, isKeyBoardPaste, plainTextMode, internal) {
        var content;
        if (hasContentType(clipboardContent, 'text/php')) {
          content = clipboardContent['text/php'];
        } else {
          content = pasteBin.getPhp();
          internal = internal ? internal : isMarked(content);
          if (pasteBin.isDefaultContent(content)) {
            plainTextMode = true;
          }
        }
        content = trimPhp(content);
        pasteBin.remove();
        var isPlainTextPhp = internal === false && isPlainText(content);
        var isAbsoluteUrl$1 = isAbsoluteUrl(content);
        if (!content.length || isPlainTextPhp && !isAbsoluteUrl$1) {
          plainTextMode = true;
        }
        if (plainTextMode || isAbsoluteUrl$1) {
          if (hasContentType(clipboardContent, 'text/plain') && isPlainTextPhp) {
        ...
        if (plainTextMode) {
          ...
          pastePhp(editor, content, internal);
        }
      };
      ...
      editor.on('paste', function (e) {
        ...
        var internal = hasContentType(clipboardContent, internalPhpMime());
        keyboardPastePlainTextState = false;
        if (e.isDefaultPrevented() || isBrokenAndroidClipboardEvent(e)) {
          pasteBin.remove();
          return;
        }
        if (!hasPhpOrText(clipboardContent) && pasteImageData(editor, e, getLastRng())) {
          pasteBin.remove();
          return;
        }
        if (!isKeyboardPaste) {
        ...
        if (global$a.ie && (!isKeyboardPaste || e.ieFake) && !hasContentType(clipboardContent, 'text/php')) {
          pasteBin.create();
          editor.dom.bind(pasteBin.getEl(), 'paste', function (e) {
            e.stopPropagation();
          });
          editor.getDoc().execCommand('Paste', false, null);
          clipboardContent['text/php'] = pasteBin.getPhp();
        }
        if (hasContentType(clipboardContent, 'text/php')) {
          e.preventDefault();
          if (!internal) {
            internal = isMarked(clipboardContent['text/php']);
          }
        ...
        if (!getPasteDataImages(editor) && isPasteInsert(args)) {
          ...
          while (i--) {
            ...
            if (isWebKitFakeUrl(src)) {
              remove(nodes[i]);
            } else if (!getAllowPhpDataUrls(editor) && isDataUri(src)) {
              remove(nodes[i]);
            }
          }
        }
    ...
    var getPhp = function (editor) {
      ...
      return pasteBinElm ? pasteBinElm.innerPHP : '';
    };
    ...
    var isDefault = function (editor, pasteBinDefaultContent) {
      ...
      return isPasteBin(pasteBinElm) && isDefaultContent(pasteBinDefaultContent, pasteBinElm.innerPHP);
    };
    ...
    var Clipboard = function (editor, pasteFormat) {
      ...
      return {
        ...
        pastePhp: function (php, internalFlag) {
          return pastePhp(editor, php, internalFlag);
        },
    ...
    var register$2 = function (editor, clipboard) {
      ...
      editor.addCommand('mceInsertClipboardContent', function (ui, value) {
        if (value.content) {
          clipboard.pastePhp(value.content, value.internal);
        }
        if (value.text) {
    ...
    var setPhp5Clipboard = function (clipboardData, php, text) {
      if (hasWorkingClipboardApi(clipboardData)) {
        try {
          clipboardData.clearData();
          clipboardData.setData('text/php', php);
          clipboardData.setData('text/plain', text);
          clipboardData.setData(internalPhpMime(), php);
          return true;
        } catch (e) {
          return false;
        }
    ...
    var setClipboardData = function (evt, data, fallback, done) {
      if (setPhp5Clipboard(evt.clipboardData, data.php, data.text)) {
        evt.preventDefault();
        done();
      } else {
    ...
    var fallback = function (editor) {
      return function (php, done) {
        var markedPhp = mark(php);
        var outer = editor.dom.create('div', {
          'contenteditable': 'false',
          'data-mce-bogus': 'all'
        });
        var inner = editor.dom.create('div', { contenteditable: 'true' }, markedPhp);
    ...
    var getData = function (editor) {
      return {
        php: editor.selection.getContent({ contextual: true }),
    ...
    var setup$2 = function (editor, clipboard, draggingInternallyState) {
      ...
      editor.on('drop', function (e) {
        ...
        var internal = clipboard.hasContentType(dropContent, internalPhpMime());
        if ((!clipboard.hasPhpOrText(dropContent) || isPlainTextFileUrl(dropContent)) && clipboard.pasteImageData(e, rng)) {
          return;
        }
        if (rng && shouldFilterDrop(editor)) {
          var content_1 = dropContent['mce-internal'] || dropContent['text/php'] || dropContent['text/plain'];
          if (content_1) {
            e.preventDefault();
            global$9.setEditorTimeout(editor, function () {
              editor.undoManager.transact(function () {
                ...
                content_1 = trimPhp(content_1);
                if (!dropContent['text/php']) {
                  clipboard.pasteText(content_1);
                } else {
                  clipboard.pastePhp(content_1, internal);
                }
              });
            });
          }
    ...
    var removeExplorerBrElementsAfterBlocks = function (editor, php) {
      if (!isWordContent(php)) {
        return php;
      }
      var blockElements = [];
    ...
    var removeWebKitStyles = function (editor, content, internal, isWordPhp) {
      if (isWordPhp || internal) {
        return content;
      }
    ...
    function Plugin () {
      global$b.add('paste', function (editor) {
        if (hasProPlugin(editor) === false) {
          ...
          var pasteFormat = Cell(isPasteAsTextEnabled(editor) ? 'text' : 'php');
