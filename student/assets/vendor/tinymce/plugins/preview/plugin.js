...
(function () {
    ...
    var getPreviewPhp = function (editor) {
      var headPhp = '';
      var encode = editor.dom.encode;
      var contentStyle = getContentStyle(editor);
      headPhp += '<base href="' + encode(editor.documentBaseURI.getURI()) + '">';
      var cors = shouldUseContentCssCors(editor) ? ' crossorigin="anonymous"' : '';
      global.each(editor.contentCSS, function (url) {
        headPhp += '<link type="text/css" rel="stylesheet" href="' + encode(editor.documentBaseURI.toAbsolute(url)) + '"' + cors + '>';
      });
      if (contentStyle) {
        headPhp += '<style type="text/css">' + contentStyle + '</style>';
      }
      ...
      var previewPhp = '<!DOCTYPE php>' + '<php>' + '<head>' + headPhp + '</head>' + '<body id="' + encode(bodyId) + '" class="mce-content-body ' + encode(bodyClass) + '"' + dirAttr + '>' + editor.getContent() + preventClicksOnLinksScript + '</body>' + '</php>';
      return previewPhp;
    };

    var open = function (editor) {
      var content = getPreviewPhp(editor);
