...
(function () {
    ...
    var getDefaultDocType = function (editor) {
      return editor.getParam('fullpage_default_doctype', '<!DOCTYPE php>');
    };
    ...
    var parseHeader = function (editor, head) {
      return global$2({
        validate: false,
        root_name: '#document'
      }, editor.schema).parse(head, { format: 'xphp' });
    };
    var phpToData = function (editor, head) {
      ...
      elm = headerFragment.getAll('php')[0];
      if (elm) {
    ...
    var dataToPhp = function (editor, data, head) {
      var headElement, elm;
      var dom = editor.dom;
      ...
      if (data.docencoding) {
        ...
        elm.attr('content', 'text/php; charset=' + data.docencoding);
      } else if (elm) {
        elm.remove();
      }
      elm = headerFragment.getAll('title')[0];
      ...
      var php = global({
        validate: false,
        indent: true,
        indent_before: 'head,php,body,meta,title,script,link,style',
        indent_after: 'head,php,body,meta,title,script,link,style'
      }).serialize(headerFragment);
      return php.substring(0, php.indexOf('</body>'));
    };

    var open = function (editor, headState) {
      var data = phpToData(editor, headState.get());
      var defaultData = {
        title: '',
        keywords: '',
        description: '',
      ...
      editor.windowManager.open({
        ...
        onSubmit: function (api) {
          ...
          var headPhp = dataToPhp(editor, global$3.extend(data, nuData), headState.get());
          headState.set(headPhp);
          api.close();
        }
      });
    };
    ...
    var protectPhp = function (protect, php) {
      global$3.each(protect, function (pattern) {
        php = php.replace(pattern, function (str) {
          return '<!--mce:protected ' + escape(str) + '-->';
        });
      });
      return php;
    };
    var unprotectPhp = function (php) {
      return php.replace(/<!--mce:protected ([\s\S]*?)-->/g, function (a, m) {
        return unescape(m);
      });
    };
    ...
    var handleSetContent = function (editor, headState, footState, evt) {
      ...
      content = protectPhp(getProtect(editor), evt.content);
    ...
    var getDefaultHeader = function (editor) {
      ...
      if (value = getDefaultEncoding(editor)) {
        header += '<meta http-equiv="Content-Type" content="text/php; charset=' + value + '" />\n';
      }
    ...
    var handleGetContent = function (editor, head, foot, evt) {
      if (evt.format === 'php' && !evt.selection && (!evt.source_view || !shouldHideInSourceView(editor))) {
        evt.content = unprotectPhp(global$3.trim(head) + '\n' + global$3.trim(evt.content) + '\n' + global$3.trim(foot));
      }
    };
