...
(function () {
    ...
    var generateTocPhp = function (editor) {
      var php = generateTocContentPhp(editor);
      return '<div class="' + editor.dom.encode(getTocClass(editor)) + '" contenteditable="false">' + php + '</div>';
    };
    var generateTocContentPhp = function (editor) {
      var php = '';
      ...
      php += generateTitle(getTocHeader(editor), global$1.translate('Table of Contents'));
      for (var i = 0; i < headers.length; i++) {
        var h = headers[i];
        h.element.id = h.id;
    ...
    var insertToc = function (editor) {
      ...
      if (isEmptyOrOffscreen(editor, $tocElm)) {
        editor.insertContent(generateTocPhp(editor));
      } else {
        updateToc(editor);
      }
    };
    var updateToc = function (editor) {
      ...
      if ($tocElm.length) {
        editor.undoManager.transact(function () {
          $tocElm.php(generateTocContentPhp(editor));
        });
      }
    };
