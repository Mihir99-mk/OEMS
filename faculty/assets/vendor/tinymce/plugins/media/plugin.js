...
(function () {
    ...
    var shouldFilterPhp = function (editor) {
      return editor.getParam('media_filter_php', true);
    };
    ...
    var phpToData = function (prefixes, php) {
    ...
    var updatePhp = function (php, data, updateAll) {
      var writer = global$4();
      var isEphoxEmbed = Cell(false);
      var sourceCount = 0;
      var hasImage;
    ...
    var getIframePhp = function (data) {
      var allowFullscreen = data.allowfullscreen ? ' allowFullscreen="1"' : '';
      return '<iframe src="' + data.source + '" width="' + data.width + '" height="' + data.height + '"' + allowFullscreen + '></iframe>';
    };
    var getFlashPhp = function (data) {
      var php = '<object data="' + data.source + '" width="' + data.width + '" height="' + data.height + '" type="application/x-shockwave-flash">';
      if (data.poster) {
        php += '<img src="' + data.poster + '" width="' + data.width + '" height="' + data.height + '" />';
      }
      php += '</object>';
      return php;
    };
    var getAudioPhp = function (data, audioTemplateCallback) {
    ...
    var getVideoPhp = function (data, videoTemplateCallback) {
    ...
    var getScriptPhp = function (data) {
      return '<script src="' + data.source + '"></script>';
    };
    var dataToPhp = function (editor, dataIn) {
      var data = global$8.extend({}, dataIn);
      if (!data.source) {
        global$8.extend(data, phpToData(getScripts(editor), data.embed));
        if (!data.source) {
          return '';
        }
      }
      ...
      if (data.embed) {
        return updatePhp(data.embed, data, true);
      } else {
        ...
        if (data.type === 'iframe') {
          return getIframePhp(data);
        } else if (data.sourcemime === 'application/x-shockwave-flash') {
          return getFlashPhp(data);
        } else if (data.sourcemime.indexOf('audio') !== -1) {
          return getAudioPhp(data, audioTemplateCallback);
        } else if (data.type === 'script') {
          return getScriptPhp(data);
        } else {
          return getVideoPhp(data, videoTemplateCallback);
        }
      }
    };
    ...
    var setup$2 = function (editor) {
      ...
      editor.on('ObjectResized', function (e) {
        ...
        if (target.getAttribute('data-mce-object')) {
          var php = target.getAttribute('data-mce-php');
          if (php) {
            php = unescape(php);
            target.setAttribute('data-mce-php', escape(updatePhp(php, {
    ...
    var cache = {};
    var embedPromise = function (data, dataToPhp, handler) {
      return new global$3(function (res, rej) {
        var wrappedResolve = function (response) {
          ...
          return res({
            url: data.source,
            php: response.php ? response.php : dataToPhp(data)
          });
        };
    ...
    var defaultPromise = function (data, dataToPhp) {
    ...
    var loadedData = function (editor) {
      return function (data) {
        return dataToPhp(editor, data);
      };
    };
    var getEmbedPhp = function (editor, data) {
    ...
    var snippetToData = function (editor, embedSnippet) {
      return phpToData(getScripts(editor), embedSnippet);
    };
    var getEditorData = function (editor) {
      ...
      return __assign({ embed: snippet }, phpToData(getScripts(editor), snippet));
    };
    var addEmbedPhp = function (api, editor) {
      return function (response) {
        if (isString(response.url) && response.url.trim().length > 0) {
          var php = response.php;
          var snippetData = snippetToData(editor, php);
    ...
    var handleInsert = function (editor, php) {
    ...
    var submitForm = function (prevData, newData, editor) {
      newData.embed = updatePhp(newData.embed, newData);
      if (newData.embed && (prevData.source === newData.source || isCached(newData.source))) {
        handleInsert(editor, newData.embed);
      } else {
        getEmbedPhp(editor, newData).then(function (response) {
    ...
    var showDialog = function (editor) {
      ...
      var handleSource = function (prevData, api) {
        ...
        if (prevData.source !== serviceData.source) {
          addEmbedPhp(win, editor)({
            url: serviceData.source,
            php: ''
          });
          getEmbedPhp(editor, serviceData).then(addEmbedPhp(win, editor)).catch(handleError(editor));
        }
      };
      ...
      var handleUpdate = function (api, sourceInput) {
        ...
        var embed = dataToPhp(editor, data);
    ...
    var sanitize = function (editor, php) {
      if (shouldFilterPhp(editor) === false) {
        return php;
      }
      var writer = global$4();
      var blocked;
    ...
    var appendNodeContent = function (editor, nodeName, previewNode, php) {
      var newNode = global({
        forced_root_block: false,
        validate: false
      }, editor.schema).parse(php, { context: nodeName });
    ...
    var createPlaceholderNode = function (editor, node) {
      ...
      retainAttributesAndInnerPhp(editor, node, placeHolder);
    ...
    var createPreviewNode = function (editor, node) {
      ...
      retainAttributesAndInnerPhp(editor, node, previewWrapper);
      ...
      if (name === 'iframe') {
        ...
        var sanitizedPhp = previewWrapper.attr('data-mce-php');
        if (isNonNullable(sanitizedPhp)) {
          appendNodeContent(editor, name, previewNode, unescape(sanitizedPhp));
        }
      }
    ...
    var retainAttributesAndInnerPhp = function (editor, sourceNode, targetNode) {
      ...
      var innerPhp = sourceNode.firstChild && sourceNode.firstChild.value;
      if (innerPhp) {
        targetNode.attr('data-mce-php', escape(sanitize(editor, innerPhp)));
      ...
      editor.on('preInit', function () {
        ...
        editor.serializer.addAttributeFilter('data-mce-object', function (nodes, name) {
          ...
          while (i--) {
            ...
            innerPhp = node.attr('data-mce-php');
            if (innerPhp) {
              innerNode = new global$2('#text', 3);
              innerNode.raw = true;
              innerNode.value = sanitize(editor, unescape(innerPhp));
