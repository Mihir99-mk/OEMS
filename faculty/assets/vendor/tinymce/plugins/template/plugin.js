...
(function () {
    ...
    var replaceTemplateValues = function (php, templateValues) {
      global$3.each(templateValues, function (v, k) {
        if (isFunction(v)) {
          v = v(k);
        }
        php = php.replace(new RegExp('\\{\\$' + k + '\\}', 'g'), v);
      });
      return php;
    };
    ...
    var insertTemplate = function (editor, _ui, php) {
      var dom = editor.dom;
      var sel = editor.selection.getContent();
      php = replaceTemplateValues(php, getTemplateReplaceValues(editor));
      var el = dom.create('div', null, php);
      ...
      global$3.each(dom.select('*', el), function (n) {
        if (hasClass(n, getCreationDateClasses(editor).replace(/\s+/g, '|'))) {
          n.innerPHP = getDateTime(editor, getCdateFormat(editor));
        }
        if (hasClass(n, getModificationDateClasses(editor).replace(/\s+/g, '|'))) {
          n.innerPHP = getDateTime(editor, getMdateFormat(editor));
        }
      ...
      editor.execCommand('mceInsertContent', false, el.innerPHP);
      editor.addVisual();
    };
    ...
    var phpEscape = function (php) {
      return php.replace(/["'<>&]/g, function (match) {
        return get(entitiesAttr, match).getOr(match);
      });
    };

    var getPreviewContent = function (editor, php) {
      if (php.indexOf('<php>') === -1) {
        ...
        php = '<!DOCTYPE php>' + '<php>' + '<head>' + '<base href="' + encode(editor.documentBaseURI.getURI()) + '">' + contentCssEntries_1 + preventClicksOnLinksScript + '</head>' + '<body class="' + encode(bodyClass) + '"' + dirAttr + '>' + php + '</body>' + '</php>';
      }
      return replaceTemplateValues(php, getPreviewReplaceValues(editor));
    };
    var open = function (editor, templateList) {
        ...
        return function (api, change) {
          if (change.name === 'template') {
            ...
            findTemplate(templates, newTemplateTitle).each(function (t) {
              api.block('Loading...');
              getTemplateContent(t).then(function (previewPhp) {
                updateDialog(api, t, previewPhp);
              }).catch(function () {
                updateDialog(api, t, '');
                api.disable('save');
                loadFailedAlert(api);
      ...
      var onSubmit = function (templates) {
        return function (api) {
          ...
          findTemplate(templates, data.template).each(function (t) {
            getTemplateContent(t).then(function (previewPhp) {
              editor.execCommand('mceInsertTemplate', false, previewPhp);
              api.close();
            }).catch(function () {
              api.disable('save');
              loadFailedAlert(api);
      ...
      var openDialog = function (templates) {
        ...
        var updateDialog = function (dialogApi, template, previewPhp) {
          var content = getPreviewContent(editor, previewPhp);
          var bodyItems = [
            {
              type: 'selectbox',
              name: 'template',
            ...
            {
              type: 'phppanel',
              php: '<p aria-live="polite">' + phpEscape(template.value.description) + '</p>'
            },
            {
              label: 'Preview',
              type: 'iframe',
        ...
        getTemplateContent(templates[0]).then(function (previewPhp) {
          updateDialog(dialogApi, templates[0], previewPhp);
        }).catch(function () {
          ...
          if (dom.hasClass(e, 'mceTmpl')) {
            global$3.each(dom.select('*', e), function (e) {
              if (dom.hasClass(e, getModificationDateClasses(editor).replace(/\s+/g, '|'))) {
                e.innerPHP = getDateTime(editor, dateFormat);
              }
            });
